...
static void dac_init(void) {
    ...
    /* Rev5 is equipped with a DAC60501 */
        ...
        // Let's check the returned value though, just in case.
        cmd = 0x04;
        i2c1_read(DAC60501_I2C_ADDR, cmd, dat, 2, true);

        printf(" -> Unnecessary DAC read returns: 0x%02x%02x\n", dat[0], dat[1]);
        if (dat[0] == 0xff && dat[1] == 0xff) {
            printf("  TCXO DAC FAIL!!!\n");
        }
...
static int xtrx_init(void)
{
    ...
    - xtrxll_open() -> xtrxll_base_dev_init()
      - XTRXLL_PARAM_PWR_CTRL, PWR_CTRL_BUSONLY
        -> calls lp8758_en(dev, en_lms = 0, en_b33 = 1)
          ...
          -> lp8758_get(dev->self, XTRX_I2C_PMIC_LMS, OTP_REV, (uint8_t*)&d);
            -> print out "PMIC_L ver %02x:%02x  en33=%d\n", v, d, en3v3);
        ...
        -> wait 100ms
      - XTRXLL_PARAM_FE_CTRL, XTRXLL_LMS7_GPWR_PIN
         -> W17 high
        -> wait 100ms
      - XTRXLL_PARAM_PWR_CTRL, PWR_CTRL_ON
        -> calls lp8758_en(dev, en_lms = 1, en_b33 = 1)
          -> _xtrxllr3_pmic_lms_set()
          -> DOES A WHOLE BUNCH OF STUFF
        -> wait 10ms
      - XTRXLL_PARAM_FE_CTRL, XTRXLL_LMS7_GPWR_PIN | XTRXLL_LMS7_RESET_PIN
        -> U19 high
        -> W17 high
        -> wait 10ms
      - lms7_enable()
        -> NO WAIT
      - XTRXLL_PARAM_FE_CTRL, XTRXLL_LMS7_GPWR_PIN | XTRXLL_LMS7_RESET_PIN | XTRXLL_LMS7_RXEN_PIN | XTRXLL_LMS7_TXEN_PIN
    - xtrx_fe_init() -> auto_init() -> lms7nfe_init()
      - 
    */
